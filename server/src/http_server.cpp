...
void handle_peers(struct mg_connection *nc, struct http_message *hm) {
    ...
    for (auto it = peer_map.begin(); it != peer_map.end();) {
        ...
        auto *peer = it_current->second;


        Json::Value json_peer;
        json_peer["clientIdentity"] = peer->identity;
        json_peer["clientIp"] = peer->src_ip;
        json_peer["srcPort"] = peer->src_port;

        if (peer->protocol == IPPROTO_ICMP) {
            json_peer["protocol"] = "icmp";
        } else if (peer->protocol == IPPROTO_TCP) {
            json_peer["protocol"] = "tcp";
        } else if (peer->protocol == IPPROTO_UDP) {
            json_peer["protocol"] = "udp";
        }

        json_peer["avgLatency"] = peer->avg_latency;
        json_peer["jitter"] = peer->jitter;
        json_peer["lossRate"] = peer->loss_rate;

        time_t time = peer->last_time / 1000;
        char buffer[128] = {'\0'};
        strftime(buffer, 128, "%Y-%m-%d %H:%M:%S", localtime(&time));
        string date_time;
        date_time.assign(buffer);
        date_time.append(".").append(to_string(peer->last_time % 1000));
        json_peer["lastRecvTime"] = date_time;
